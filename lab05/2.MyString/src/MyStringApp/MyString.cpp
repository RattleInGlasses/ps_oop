...
void CMyString::SetCapacity(size_t requiredSize)
{
	...
	if (!m_pData)
	{
		m_pData = make_unique<char[]>(newSize);
		m_capacity = newSize;
		*m_pData.get() = '\0';
	}	
...
CMyString::iterator &CMyString::iterator::operator +=(ptrdiff_t offset)
{
	*this = *this + offset;
	return *this;
}
...
CMyString::iterator &CMyString::iterator::operator =(CMyString::iterator const &it2)
{
	if (m_pString == it2.m_pString)
	{
		m_pChar = it2.m_pChar;
	}
	return *this;
...
CMyString::const_iterator &CMyString::const_iterator::operator +=(ptrdiff_t offset)
{
	*this = *this + offset;
	return *this;
}
...
CMyString::const_iterator &CMyString::const_iterator::operator =(CMyString::const_iterator const &it2)
{
	if (m_pString == it2.m_pString)
	{
		m_pChar = it2.m_pChar;
